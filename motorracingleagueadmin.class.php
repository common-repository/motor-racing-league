...
class MotorRacingLeagueAdmin extends MotorRacingLeague
{
	...
	function activate() {
		...
		add_option('motorracingleague_notice_panel' , 1);
...
<?php
	...
	function register_mysettings() { // whitelist options
		...
		register_setting( $this->pf.'option-group', 'motorracingleague_notice_panel' );
...
?>
	...
	<tr valign="top">
	<th scope="row"><?php _e('Notice panel position', $this->name) ?></th>
	<?php
	$opts = array(1 => __('Top', $this->name),
				  2 => __('Bottom', $this->name),
				  3 => __('Both', $this->name),
				  );
	?>
	<td><select name="motorracingleague_notice_panel">
		<?php
		$selected = get_option('motorracingleague_notice_panel');
	...
	function getStandings($champid, $raceid) {
		...
		foreach ($entry as $row) {
			if ($group_id != $row->id) {
				...
				$output .= '<td data-position="'.$row->position.'">' . $this->pts($row->shortcode, $row->shortcode_name, $points_breakdown, $row->position) . "</td>";
			} else {
				$nextrow=true;
				$output .= '<td data-position="'.$row->position.'">' . $this->pts($row->shortcode, $row->shortcode_name, $points_breakdown, $row->position) . "</td>";
			}
